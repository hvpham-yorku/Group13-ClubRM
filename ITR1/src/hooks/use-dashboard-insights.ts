 ...
 *
 * Client-side computation of all 6 insight panel data objects from live
 * context providers (members, finance, events, tasks). This hook mirrors
 * the logic in api/stats.ts so insight panels always show live data —
 * even on local dev where the Vercel serverless function is unavailable.
 *
 * Usage:
 *   const { orgHealth, members, budget, events, risks, approvals } = useDashboardInsights()
 */
...
import { calculateHealthScore } from "@/lib/dashboard-logic"
import type {
  OrgHealthInsight,
  MembersInsight,
  BudgetInsight,
  ...
  RisksInsight,
  ApprovalsInsight,
} from "@/components/dashboard/insights/types"

/** Group an array by a key and return label/count/percentage slices. */
function groupCount<T>(items: T[], key: keyof T) {
...
/** Build category spend breakdown from approved expenses and total budget. */
function buildCategorySpend(approvedEntries: { amount: number; category: string }[], totalBudget: number) {
...
/**
 * Computes all 6 dashboard insight objects from live context data.
...
export function useDashboardInsights(): DashboardInsights {
  ...
  const { expenses, reimbursements, budget, totalSpent } = useFinance()
  ...
  return useMemo(() => {
    ...
    const newThisTerm = allMembers.filter((m) => new Date(m.joinDate) >= termStart).length
    const avgTasksCompleted =
      totalMembers > 0
        ? Math.round((allMembers.reduce((s, m) => s + (m.tasksCompleted || 0), 0) / totalMembers) * 10) / 10
        : 0
    const avgEventsAttended =
      totalMembers > 0
        ? Math.round((allMembers.reduce((s, m) => s + (m.eventsAttended || 0), 0) / totalMembers) * 10) / 10
        : 0

    const recentJoiners = [...allMembers]
      .sort((a, b) => new Date(b.joinDate).getTime() - new Date(a.joinDate).getTime())
      .slice(0, 3)
      .map((m) => ({ name: m.name, role: m.role, department: m.department, joinDate: m.joinDate }))
    ...
    const inactiveWarnings = allMembers
      ...
      .slice(0, 3)
      .map((m) => ({ name: m.name, lastActive: m.joinDate, reason: `Status: ${m.status}` }))

    const membersInsight: MembersInsight = {
      totalMembers,
      activeMembers,
      newThisTerm,
    ...
    const allApprovedEntries = approvedExpenses.map((e) => ({ amount: e.amount, category: e.category }))
    const categories = buildCategorySpend(allApprovedEntries, totalBudget)
    ...
    if (percentRemaining < 20)
      budgetAlerts.push({ label: "Budget critically low", status: "critical", detail: `Only ${percentRemaining}% remaining ($${remainingBudget.toLocaleString()})` })
    else if (percentRemaining < 40)
      budgetAlerts.push({ label: "Budget below 40%", status: "warning", detail: `${percentRemaining}% remaining — monitor closely` })
    else
      budgetAlerts.push({ label: "Budget healthy", status: "good", detail: `${percentRemaining}% remaining ($${remainingBudget.toLocaleString()})` })
    if (burnRate > targetBurnRate * 1.2)
      budgetAlerts.push({ label: "Burn rate above target", status: "warning", detail: `$${burnRate}/mo vs $${targetBurnRate}/mo target` })
    ...
    const onTrackEvents = allEvents.filter((e) => {
      const reg = e.registered || 0
      const cap = e.capacity || 0
      return cap === 0 || reg / cap >= 0.5
    }).length

    const eventsInsight: EventsInsight = {
      totalUpcoming,
      avgRegistrationRate: avgRegRate,
      volunteerCoverageRate: 100,
      events: eventDetails,
      trend: monthlySpend.map((m) => ({ label: m.label, value: Math.round(avgRegRate * (0.9 + Math.random() * 0.2)) })),
      strengths: totalUpcoming > 0 ? [`${totalUpcoming} upcoming event(s) scheduled`] : ["No upcoming events — schedule new events soon"],
    ...
    const pendingAmount =
      ...
      reimbursements.filter((r) => r.status === "pending").reduce((s, r) => s + r.amount, 0)

    const risksList: RisksInsight["risks"] = []
    if (overdueTasks.length > 0) {
      risksList.push({
        title: `${overdueTasks.length} overdue task(s)`,
        category: "tasks",
        severity: overdueTasks.length >= 3 ? "high" : "medium",
        description: `Tasks past due: ${overdueTasks.map((t) => t.title).slice(0, 3).join(", ")}`,
    ...
    if (pendingAmount > 500) {
      risksList.push({
        ...
        description: `$${pendingAmount.toFixed(0)} in pending expense/reimbursement approvals awaiting action.`,
    ...
    eventDetails.filter((e) => e.status === "critical").forEach((e) => {
      risksList.push({
        ...
        category: "events",
        severity: "high",
        description: `${e.registered}/${e.capacity} registered — below 40% fill rate.`,
    ...
    if (percentRemaining < 20) {
      risksList.push({
        ...
        category: "budget",
        severity: "high",
        description: `Only ${percentRemaining}% of the budget remains ($${remainingBudget.toLocaleString()}).`,
    ...
    const risksInsight: RisksInsight = {
    ...
    const approvalItems: ApprovalsInsight["items"] = [
      ...pendingExpenses.slice(0, 5).map((e) => ({
        id: e.id,
        type: "finance" as const,
        title: e.description,
        submittedBy: e.submittedBy,
        submittedAt: new Date(e.date).toISOString().split("T")[0],
        daysWaiting: Math.floor((now.getTime() - new Date(e.date).getTime()) / 86400000),
        priority: e.amount > 500 ? ("high" as const) : ("medium" as const),
        amount: e.amount,
      ...
      ...pendingReimbs.slice(0, 3).map((r) => ({
        ...
        submittedAt: new Date(r.date).toISOString().split("T")[0],
        daysWaiting: Math.floor((now.getTime() - new Date(r.date).getTime()) / 86400000),
        priority: r.amount > 200 ? ("high" as const) : ("medium" as const),
        amount: r.amount,
        description: `Reimbursement request — $${r.amount}`,
      })),
    ]

    const approvalsInsight: ApprovalsInsight = {
      ...
      recentlyApproved: [...approvedExpenses]
        ...
        .slice(0, 3)
        .map((e) => ({ title: e.description, approvedAt: new Date(e.date).toISOString().split("T")[0], type: "Finance" })),
    }
    ...
    const eventScore = allEvents.length > 0 ? Math.round((onTrackEvents / allEvents.length) * 25) : 25
    const taskScore = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 25) : 25

    const orgHealthInsight: OrgHealthInsight = {
      overallScore: score,
      ...
      breakdowns: [
        {
          ...
          insights: [
            {
              label: `${activeMembers} of ${totalMembers} members active`,
              status: activeMembers / (totalMembers || 1) >= 0.8 ? "good" : "warning",
              detail: `Active ratio: ${Math.round((activeMembers / (totalMembers || 1)) * 100)}%`,
            },
            {
              label: `${newThisTerm} new member(s) this term`,
              status: "good",
              detail: "Based on join date in past 4 months",
            },
          ],
        },
        {
          ...
          insights: [
            {
              label: `${percentRemaining}% budget remaining`,
              status: percentRemaining >= 40 ? "good" : "warning",
              detail: `$${spentBudget.toLocaleString()} of $${totalBudget.toLocaleString()} spent`,
            },
            {
              label: `Burn rate $${burnRate}/mo`,
              status: burnRate <= targetBurnRate ? "good" : "warning",
              detail: `Target: $${targetBurnRate}/mo`,
            },
          ],
        },
        {
          ...
          insights: [
            {
              label: `${onTrackEvents} of ${allEvents.length} events on track`,
              status: eventScore >= 18 ? "good" : "warning",
              detail: "Based on registration vs. capacity",
            },
            {
              label: `${totalUpcoming} upcoming event(s)`,
              status: "good",
              detail: "Scheduled in calendar",
            },
          ],
        },
        {
          ...
          insights: [
            {
              ...
              status: taskScore >= 18 ? "good" : "warning",
              detail: `${Math.round((completedTasks / (totalTasks || 1)) * 100)}% completion rate`,
            },
            {
              label: `${overdueTasks.length} overdue task(s)`,
              status: overdueTasks.length === 0 ? "good" : "warning",
              detail:
      ...
      topStrengths: [
        ...
        totalUpcoming > 0 ? `${totalUpcoming} upcoming event(s) actively planned` : null,
      ...
      topConcerns: [
        ...
        pendingAmount > 1000 ? `$${pendingAmount.toFixed(0)} in pending approvals` : null,
      ].filter(Boolean) as string[],
    }
  ...
  }, [allMembers, expenses, reimbursements, budget, totalSpent, allEvents, allTasks])
}
