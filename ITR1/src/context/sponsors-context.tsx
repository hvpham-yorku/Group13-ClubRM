...
// Helper interface for parsing JSON from Supabase
interface RawContact {
  id?: string;
  name?: string;
  email?: string;
  phone?: string;
  title?: string; // Matches 'title' in your types.ts
}
  ...
  return {
    ...
    contacts: (() => {
      ...
      return parsed.map((c: RawContact): SponsorContact => ({
        ...
        email: c.email || "",
        phone: c.phone || "",
        title: c.title || "Contact", // Maps to the correct property in types.ts
      }))
    })(),
    interactions: (() => {
